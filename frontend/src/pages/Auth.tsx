...
export function Component() {
  ...
  const isAuthBusy = loading || googlePending;
  const googleLabel = isLogin
    ? 'Tiếp tục với Google'
    : 'Đăng ký nhanh với Google';
  const googleHint = googlePending
    ? 'Vui lòng chờ trong giây lát...'
    : isLogin
      ? 'Dùng Google để đăng nhập an toàn và nhanh hơn'
      : 'Tạo tài khoản mới bằng Google chỉ trong vài giây';
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  ...
                  className="group flex w-full cursor-pointer items-center gap-3 rounded-xl border border-border bg-surface px-4 py-3 text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-border-strong hover:bg-surface-alt hover:shadow-sm focus-visible:border-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/15 active:translate-y-0 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60"
                  onClick={handleGoogleAuth}
                >
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-border bg-surface-alt shadow-sm transition-colors group-hover:border-border-strong group-hover:bg-surface">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block font-display text-sm font-semibold text-text-primary">
                      {googlePending ? 'Đang chuyển đến Google' : googleLabel}
                    </span>
