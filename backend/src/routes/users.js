 ...
 * Thiết lập mật khẩu (cho user đăng nhập bằng Google lần đầu).
 */
usersRouter.post("/me/setup-password", requireAuth, async (req, res) => {
  const { newPassword } = req.body;

  if (!newPassword) {
    return res.status(400).json(fail("Vui lòng nhập mật khẩu mới", 400));
  }
  ...
  try {
    ...
    if (mongoUser) {
      mongoUser.password = newPassword; // Sẽ tự hash qua pre-save hook
      mongoUser.hasPassword = true;
      mongoUser.authProvider = mongoUser.authProvider === "GOOGLE" ? "GOOGLE_AND_LOCAL" : "LOCAL";
      await mongoUser.save();

      return res.json(ok(null, "Thiết lập mật khẩu thành công"));
    }
  } catch {
    // Fallback: in-memory store
  }
  ...
  res.json(ok(null, "Thiết lập mật khẩu thành công"));
});
